...
> For a vector space, $V$, an *inner product* is a function from $V\times V\to\mathbb R$, notated as $\langle,\rangle$ that meets the following criteria
>
> a) For all $v\in V$, $\langle v,v\rangle\geq0$^[positivity of inner products]
b) For all $v\in V$,  $\langle v,v\rangle=0 \iff v=\textbf{0}$
c) For all $u,v\in V$, $\langle u,v\rangle=\langle v,u\rangle$^[symmetry of inner products]
d) For all $u,v\in V$ and $c\in\mathbb R$, $\langle cu,v\rangle=c\langle u,v\rangle$
e) For all $u,v,w\in V \langle u+v, w\rangle = \langle u,w\rangle + \langle v,w\rangle$^[d and e represent the linearity of inner products]
>
> A vector space equipped with an inner product is called and *Inner Product Space*.

The following theorem aids in our convenience of using algebra on inner products.

:::{.callout-note}
## Theorem: Inner Product Properties
Suppose $V$ is an inner product space, $u,v,w\in V$, and $c\in\mathbb R$. Then,

1) $\langle\textbf{0}, v\rangle = \langle v, \textbf{0}\rangle=0$
2) $\langle u, v+w\rangle = \langle u,v\rangle + \langle u, w\rangle$
3) $\langle u, cv\rangle=c\langle u,v\rangle$
:::
...
Observe that for all $v\in V$,

\begin{align}
\langle v,\textbf{0}\rangle &= \langle v,\textbf{0} + \textbf{0}\rangle\\
                            &= \langle\textbf{0} + \textbf{0}, v\rangle\\
                            &= \langle\textbf{0}, v\rangle + \langle\textbf{0},v\rangle\\
                            &= \langle v,\textbf{0}\rangle + \langle v,\textbf{0}\rangle 
\end{align}

Thus, $\langle v,\textbf{0}\rangle = \langle v,\textbf{0}\rangle + \langle v,\textbf{0}\rangle$ which means that $0=\langle v,\textbf{0}\rangle=\langle\textbf{0}, v\rangle$ which is what we wanted to show.
...
:::{.callout-tip}
## Inner Products of Large Sums
Induction can be used to prove similar properties with sums of more than two vectors. For example, if $u,v_1,...,v_n\in V$ for some inner product space $V$, then $$\left\langle u,\sum\limits_{i=1}^nv_i\right\rangle = \sum\limits_{i=1}^n\langle u,v_i\rangle$$
:::

## Norms

> Let $V$ be an inner product space. Then, for $v\in V$, the *norm* of $V$, notated as $\|v\|$ is defined^[In some texts, the term here is the *induced norm* by the inner product, and the notion of a norm is generalized to be a function that has a particular set of properties, including those proved in the following theorem.] to be $\|v\|=\sqrt{\langle v,v\rangle}$

:::{.callout-tip}
## What is a norm?

Norms generalize the idea of a vector's length. In $\mathbb R^n$ for $n=1,2,3$, the norm of a vector actually is its length! However, in other spaces, the norm of a vector is more emblematic of the vector's length rather than a geometrical measurement.
:::

:::{.callout-note}
## Theorem: Norm Properties
Let $V$ be an inner product space with $u,v\in V$. $c\in\mathbb R$. Then
...
\begin{align}
\|cv\| &= \sqrt{\langle cv,cv\rangle}\\
       &= \sqrt{c^2\langle v,v\rangle}\\
       &= \sqrt{c^2}\sqrt{\langle v,v\rangle}\\
       &= |c|\|v\|
\end{align}
...
So $0=\sqrt{\langle v,v\rangle}$, squaring both sides gives us $0=\langle v,v\rangle$ which means that $v=\textbf{0}$ by definition of an inner product.

If $v=\textbf{0}$, then

\begin{align}
\|v\| &= \sqrt{\langle v,v\rangle}\\
      &= \sqrt{\langle\textbf{0},\textbf{0}\rangle}\\
      &=\sqrt{0}\\
      &=0
\end{align}
$\blacksquare$
...
> Let $V$ be an inner product space. Then two vectors, $u,v\in V$ are called *orthogonal* if $\langle u,v\rangle=0$.

:::{.callout-tip}
## What is orthogonality?

Orthogonality generalizes the idea of vectors being perpendicular in a similar way that norms generalize length.
:::

:::{.callout-tip}
## Zero Vector and Orthogonality

The zero vector is orthogonal to every vector and is the only vector to be orthogonal to itself. These follow from the properties and definitions of an inner product, respectively.
:::

:::{.callout-note}
## Pythagorean Theorem

Let $V$ be an inner product space where $u,v\in V$ are orthogonal. Then $$\|u+v\|^2 = \|u\|^2 \|v\|^2$$
:::
...
Let $V$ be an inner product space with $u,v\in V$. Then $|\langle u,v\rangle|\leq\|u\|\|v\|$. 
:::

:::{.callout-note collapse=true}
## Proof
If $v=\textbf{0}$, then $$|\langle u,v\rangle| = |\langle u,\textbf{0}\rangle|=|0|=0$$ Similarly, $$\|u\|\|v\|=\|u\|\|\textbf{0}\|=\|u\|*0=0$$ So the inequality is an equality in the case where $v=\textbf{0}$.

If $v\neq\textbf{0}$, then $\|v\|\neq0$. Define $w=u-\frac{\langle u,v\rangle}{\|v\|^2}v$. Observe that

\begin{align}
\langle v,w\rangle &= \left\langle v, u-\frac{\langle u,v\rangle}{\|v\|^2}v\right\rangle\\
                   &= \langle v,u\rangle - \left\langle v, \frac{\langle u,v\rangle}{\|v\|^2}v\right\rangle\\
...
So $v,w$ are orthogonal and so are $cv, w$ for any $c\in\mathbb R$ since $$\langle cv,w\rangle=c\langle v,w\rangle = c*0=0$$ In this case we choose $c=\frac{\langle u,v\rangle}{\|v\|^2}$ and observe that $w=u-cv$. Using the pythagorean theorem, observe that

\begin{align}
\|u\|^2 &= \|cv + (u-cv)\|^2\\
        &= \|cv + w\|^2\\
        &= \|cv\|^2 + \|w\|^2\\
        &= \left(|c|\|v\|\right)^2 + \|w\|^2\\
        &= |c|^2\|v\|^2 + \|w\|^2\\
        &= c^2\|v\|^2 + \|w\|^2\\
        &\geq c^2\|v\|^2
\end{align}
...
\begin{align}
\|u\|^2\geq c^2\|v\|^2 &\implies \|u\|^2\geq \left(\frac{\langle u,v\rangle}{\|v\|^2}\right)^2\|v\|^2\\
                       &\implies \|u\|^2\geq\frac{\langle u,v\rangle^2\|v\|^2}{\|v\|^4}\\
                       &\implies \|u\|^2\geq\frac{\langle u,v\rangle^2}{\|v\|^2}\\
                       &\implies \|u\|^2\|v\|^2\geq\langle u,v\rangle^2\\
                       &\implies \left(\|u\|\|v\|\right)^2\geq\langle u,v\rangle^2\\
                       &\implies \|u\|\|v\|\geq|\langle u,v\rangle|
\end{align}
...
Let $V$ be an inner product space with $u,v\in V$. Then $\|u+v\|\leq\|u\|+\|v\|$
:::

:::{.callout-note collapse=true}
## Proof
We start from the Cauchy-Schwartz Inequality and continue from there, noting that $\langle u,v\rangle\leq|\langle u,v\rangle|$

\begin{align}
|\langle u,v\rangle|\leq\|u\|\|v\| &\implies \langle u,v\rangle\leq\|u\|\|v\|\\
                                   &\implies 2\langle u,v\rangle\leq2\|u\|\|v\|\\
                                   &\implies \langle u,u\rangle + 2\langle u,v\rangle + \langle v,v\rangle \leq \|u\|^2 + 2\|u\|\|v\| + \|v\|^2\\
                                   &\implies \langle u,u\rangle + \langle u,v\rangle + \langle v,u\rangle + \langle v,v\rangle\leq\left(\|u\|+\|v\|\right)^2\\
                                   &\implies \langle u, u+v\rangle + \langle v,u+v\rangle \leq\left(\|u\|+\|v\|\right)^2\\
                                   &\implies \langle u+v, u+v\rangle +\leq\left(\|u\|+\|v\|\right)^2\\
                                   &\implies \|u+v\|^2\leq\left(\|u\|+\|v\|\right)^2\\
                                   &\implies \|u+v\|\leq\|u\|+\|v\|
                                   &&\blacksquare
\end{align}
:::
...
>Let $V$ be an inner product space and let $v,w\in V$. Then, the *distance* between $v$ and $w$ is notated and defined as $$\text{dist}(v, w) = \|v-w\|$$

:::{.callout-note}
## Theorem: Properties of a Metric
Let $V$ be an inner product space and let $x,y,z\in V$. Then the following are true:
...
(4) $\text{dist}(x,z) \leq\text{dist}(x,y) + \text{dist}(y,z)$
:::
...
> Let $V$ be an inner product space. Then a set, $\{v_1,...,v_n\}\subseteq V$ is called an *orthogonal set* if each vector is pairwise orthogonal. That is, $\langle v_i,v_j\rangle=0$ when $i\neq j$

### Orthonormal Sets

> A set is called an *orthonormal set* if it is an orthogonal set and the norm of each vector in the set is 1.

:::{.callout-tip}
The zero vector cannot be contained in an orthonormal set since the norm of the zero vector is 0.
:::

:::{.callout-note}
## Theorem: Orthogonality and Span
Let $V$ be an inner product space and let $\{b_1,...,b_n\}\subseteq V$ be an orthonormal set. Then any vector is orthogonal to anything in the span of the remaining vectors. That is $$\langle b_i, v\rangle=0$$ when $$v\in\text{span}(\{b_j: j\neq i\})$$
:::

:::{.callout-note collapse=true}
## Proof
Without loss of generality let $i=n$. Let $c_1,...,c_{n-1}\in\mathbb R$. Then observe that

\begin{align}
\left\langle\sum\limits_{i=1}^{n-1}c_ib_i, b_n\right\rangle &=\sum\limits_{i=1}^{n-1}\left\langle c_ib_i, b_n\right\rangle\\
&= \sum\limits_{i=1}^{n-1}c_i\left\langle b_i,b_n\right\rangle\\
&= \sum\limits_{i=1}^{n-1}(c_i*0)\\
&= \sum\limits_{i=1}^{n-1}0\\
&= 0
\end{align}

Thus, $b_n$ is orthogonal to anything in the span of the remaining vectors. Since the choice of $i=n$ was arbitrary, this proves the theorem.
:::

:::{.callout-note}
## Expanded Pythagorean Theorem
Let $V$ be an inner product space and let $\{b_1,...,b_n\}\subseteq V$ be an orthonormal set and let $c_1,...,c_n\in\mathbb R$. Then $$\left\|\sum\limits_{i=1}^nc_ib_i\right\|^2 = \sum\limits_{i=1}^nc_i^2$$
:::

:::{.callout-note collapse=true}
## Proof
We will prove this by induction on $n$, the number of vectors in the orthonormal set.

In the base case with $n=1$, observe that

\begin{align}
\left\|\sum\limits_{i=1}^1c_ib_i\right\|^2 &= \|c_1b_1\|^2\\
                                         &= |c_1|^2\|b\|^2\\
                                         &= |c_1|^2*1^2\\
                                         &= c_1^2\\
                                         &= \sum\limits_{i=1}^1c_i^2
\end{align}

For the induction hypothesis, suppose the theorem is true for some $k\in\mathbb N$. Then observe

\begin{align}
\left\|\sum\limits_{i=1}^{k+1}c_ib_i\right\|^2 &= \left\|c_{k+1}b_{k+1}+\sum\limits_{i=1}^{k}c_ib_i\right\|^2\\
&= \|c_{k+1}b_{k+1}\|^2 + \left\|\sum\limits_{i=1}^{k}c_ib_i\right\|^2\\
&= |c_{k+1}|^2\|b_{k+1}\|^2 + \sum\limits_{i=1}^k|c_i|^2\\
&= |c_{k+1}|^2*1^2 + \sum\limits_{i=1}^k|c_i|^2\\
&= |c_{k+1}|^2+ \sum\limits_{i=1}^k|c_i|^2\\
&= \sum\limits_{i=1}^{k+1}|c_i|^2\\
&= \sum\limits_{i=1}^{k+1}c_i^2
&&\blacksquare
\end{align}
:::
...
:::{.callout-note}
## Theorem: Linear Independence and Orthonormality
An orthonormal set is a linearly independent set.
:::

:::{.callout-note collapse=true}
## Proof
Let $\{b_1,...,b_n\}$ be an orthonormal set, and let $c_1,...,c_n\in\mathbb R$ such that $$\sum\limits_{i=1}^{n}c_ib_i=\textbf{0}$$ Observe that

\begin{align}
\sum\limits_{i=1}^{n}c_ib_i=\textbf{0} &\implies \left\|\sum\limits_{i=1}^{n}c_ib_i\right\|^2 = \|\textbf{0}\|^2\\
&\implies\sum\limits_{i=1}^nc_i^2=0\\
\end{align}

We take care to note that $\sum\limits_{i=1}^nc_i^2$ is the sum of non-negative terms. Since this sum equals 0, that means that every term in the sum is zero. Observe that


\begin{align}
|c_i|^2 = 0 &\implies c_i^2=0\\
&\implies c_i=0
\end{align}

Thus, the orthonormal set is linearly independent, which is what we wanted to show. $\blacksquare$
:::

## The Orthonormal Basis

>An *orthogonal basis* is a basis that is also an orthogonal set. An *orthonormal basis* is a basis that is also an orthonormal set.

To see why an orthonormal basis is useful, let $V$ be an inner product space and let $\{u_1,...,u_n\}$ be an orthonormal basis of $V$.

Then, if $v\in V$, then there exist unique $c_1,...,c_n$ such that $$v=\sum\limits_{i=1}^n c_iu_i$$ Observe that

\begin{align*}
\|v\| &= \left\|\sum\limits_{i=1}^n c_iu_i\right\|\\
      &= \sum\limits_{i=1}^n\|c_iu_i\| \text{ (by the extended pythagorean theorem)}\\
      &= \sum\limits_{i=1}^n|c_i|\|u_i\|\\
      &= \sum\limits_{i=1}^n|c_i|*1\\
      &= \sum\limits_{i=1}^n|c_i|
\end{align*}

Thus, if we know a vector space has an orthonormal basis, the norm of every vector is easy to calculate if we know its coordinates. Fortunately, that is easy to calculate too.

\begin{align*}
v = \sum\limits_{i=1}^n c_iu_i &\implies \langle u_j,v\rangle = \left\langle u_j,\sum\limits_{i=1}^n c_iu_i\right\rangle\\
&\implies \langle u_j,v\rangle = \sum\limits_{i=1}^n\langle u_j, c_iu_i\rangle\\
&\implies \langle u_j,v\rangle = \sum\limits_{i=1}^nc_i\langle u_j, u_i\rangle\\
&\implies \langle u_j,v\rangle = \sum\limits_{i=j}^nc_i\langle u_j, u_i\rangle + \sum\limits_{i\neq j}^nc_i\langle u_j, u_i\rangle\\
&\implies \langle u_j,v\rangle = c_j\langle u_j,u_j\rangle + \sum\limits_{i\neq j}^nc_i*0\\
&\implies \langle u_j,v\rangle = c_j\|u_j\|^2 + \sum\limits_{i\neq j}^n0\\
&\implies \langle u_j,v\rangle = c_j*1^2 + (n-1)*0\\
&\implies \langle u_j,v\rangle = c_j
\end{align*}

Thus, $v=\sum\limits_{i=1}^n\langle u_i,v\rangle u_i$ and $\|v\|=\sum\limits_{i=1}^n|\langle u_i,v\rangle|$.

Given the usefulness of an orthonormal basis, it is helpful to know how to convert a basis into an orthonormal basis.

:::{.callout-note}
## Theorem: The Grahm-Schmidt Process

Let $V$ be an inner product space and let $\{v_1,...,v_n\}$ be a basis of $V$. Define the set of vectors, $\{f_1,...,f_n\}$ such that $f_1=v_1$ and $$f_k=v_k-\sum\limits_{i=1}^{k-1}\frac{\langle v_k, f_i\rangle}{\|f_i\|^2}f_i$$ for $k=2,...,n$. Then, define the set of vectors $\{e_1,...,e_n\}$ such that $e_i=\frac{f_i}{\|f_i\|}$. Then $\{e_1,...,e_n\}$ is an orthonormal basis of $V$ and for all $k=1,...,n$, $$\text{span}(v_1,...,v_k)=\text{span}(e_1,...,e_k)$$
:::

:::{.callout-note collapse=true}
## Proof^[See Theorem 6.32 of Linear Algebra Done Right for more details]
When $k=1$, $e_k=\frac{v_k}{\|v_k\|}\implies\|v_k\|e_k=v_k$, so $e_k$ and $v_k$ are in the span of the other since they are each scalar multiples of each other. Observe that

\begin{align*}
\|e_k\| &= \left\|\frac{v_k}{\|v_k\|}\right\|\\
        &= \frac{1}{|\|v_k\||}\|v_k\|\\
        &= \frac{\|v_k\|}{\|v_k\|}\\
        &= 1
\end{align*}

Where we know that none of the $v_k$ are the zero vector since they are elements of a basis. Hence, $\{e_1\}$ is an orthonormal set.

Now, suppose for some $k<n$ that $\{e_1,...,e_k\}$ is an orthonormal set and that $\text{span}(e_1,...,e_k)=\text{span}(v_1,...,v_k)$. Since $v_{k+1}\notin\text{span}(v_1,...,v_k)=\text{span}(f_1,...,f_k)$, then $f_{k+1}$ cannot be zero (since otherwise $f_{k+1}$ is defined by the coordinates of a vector as if $\{v_1,...,v_k\}$ is a basis of $f$, see more on this idea in the next section). Observe that

\begin{align*}
\|e_k\| &= \left\|\frac{f_{k+1}}{\|f_{k+1}\|}\right\|\\
        &= \frac{1}{|\|f_{k+1}\||}\|f_{k+1}\|\\
        &= \frac{\|f_{k+1}\|}{\|f_{k+1}\|}\\
        &= 1
\end{align*}
...
\begin{align*}
\langle e_j, e_{k+1}\rangle &= \left\langle e_j,\frac{f_{k+1}}{\|f_{k+1}\|}\right\rangle\\
&=\frac{1}{\|f_{k+1}\|}\langle e_j, f_{k+1}\rangle\\
&=\frac{1}{\|f_{k+1}\|}\left\langle e_j, v_{k+1}-\sum\limits_{i=1}^{k}\frac{\langle v_{k+1}, f_i\rangle}{\|f_i\|^2}f_i\right\rangle\\
&=\frac{1}{\|f_{k+1}\|}\left\langle e_j, v_{k+1}-\sum\limits_{i=1}^{k}\frac{\langle v_{k+1}, \|f_i\|e_i\rangle}{\|f_i\|^2}f_i\right\rangle\\
&=\frac{1}{\|f_{k+1}\|}\left\langle e_j, v_{k+1}-\sum\limits_{i=1}^{k}\frac{\|f_i\|\langle v_{k+1}, e_i\rangle}{\|f_i\|^2}f_i\right\rangle\\
&=\frac{1}{\|f_{k+1}\|}\left\langle e_j, v_{k+1}-\sum\limits_{i=1}^{k}\frac{\langle v_{k+1}, e_i\rangle}{\|f_i\|}f_i\right\rangle\\
&=\frac{1}{\|f_{k+1}\|}\left(\langle e_j, v_{k+1}\rangle - \left\langle e_j,\sum\limits_{i=1}^{k}\frac{\langle v_{k+1}, e_i\rangle}{\|f_i\|}\|f_i\|e_i\right\rangle\right)\\
&=\frac{1}{\|f_{k+1}\|}\left(\langle e_j, v_{k+1}\rangle - \left\langle e_j,\sum\limits_{i=1}^{k}\langle v_{k+1}, e_i\rangle e_i\right\rangle\right)\\
&=\frac{1}{\|f_{k+1}\|}\left(\langle e_j, v_{k+1}\rangle - \sum\limits_{i=1}^{k}\left\langle e_j,\langle v_{k+1}, e_i\rangle e_i\right\rangle\right)\\
&= \frac{1}{\|f_{k+1}\|}\left(\langle e_j, v_{k+1}\rangle - \sum\limits_{i=1}^{k}\langle v_{k+1}, e_i\rangle\left\langle e_j, e_i\right\rangle\right)\\
&= \frac{1}{\|f_{k+1}\|}\left(\langle e_j, v_{k+1}\rangle - \sum\limits_{i=j}^{k}\langle v_{k+1}, e_i\rangle\left\langle e_j, e_i\right\rangle- \sum\limits_{i\neq j}^{k}\langle v_{k+1}, e_i\rangle\left\langle e_j, e_i\right\rangle\right)\\
&= \frac{1}{\|f_{k+1}\|}\left(\langle e_j, v_{k+1}\rangle - \langle v_{k+1}, e_j\rangle\left\langle e_j, e_j\right\rangle- \sum\limits_{i\neq j}^{k}\langle v_{k+1}, e_i\rangle*0\right)\\
&= \frac{1}{\|f_{k+1}\|}\left(\langle e_j, v_{k+1}\rangle - \langle e_j,v_{k+1}\rangle\|e_j\|^2- \sum\limits_{i\neq j}^{k}0\right)\\
&= \frac{1}{\|f_{k+1}\|}\left(\langle e_j, v_{k+1}\rangle - \langle e_j,v_{k+1}\rangle* 1^2- (k-1)*0\right)\\
&= \frac{1}{\|f_{k+1}\|}\left(\langle e_j, v_{k+1}\rangle - \langle e_j,v_{k+1}\rangle* 1- 0\right)\\
&=\frac{1}{\|f_{k+1}\|}\left(\langle e_j, v_{k+1}\rangle - \langle e_j,v_{k+1}\rangle\right)\\
&=\frac{1}{\|f_{k+1}\|}*0\\
&=0
\end{align*}

Thus, $\{e_1,...,e_{k+1}\}$ is an orthonormal set. It remains to be shown that $$\text{span}(e_1,...,e_{k+1})=\text{span}(v_1,...,v_{k+1})$$

Suppose $b\in \text{span}(e_1,...,e_{k+1})$. Then $$b=\sum\limits_{i=1}^{k+1}\langle b, e_i\rangle e_i = \langle b, e_{k+1}\rangle e_{k+1} + \sum\limits_{i=1}^{k}\langle b, e_i\rangle e_i$$ Then define $$a = \sum\limits_{i=1}^{k}\langle b, e_i\rangle e_i\in\text{span}(e_1,...,e_k)=\text{span}(v_1,...,v_k)\subset\{v_1,...,v_{k+1}\}$$ So $b=a + \langle b, e_{k+1}\rangle e_{k+1}$ Then, observe that 

$$
\begin{align*}
\langle b, e_{k+1}\rangle e_{k+1} &= \langle b, e_{k+1}\rangle \frac{1}{\|f_{k+1}\|}f_{k+1}\\
&= \frac{\langle b, e_{k+1}\rangle}{\|f_{k+1}\|}\left(v_{k+1}-\sum\limits_{i=1}^{k}\frac{\langle v_{k+1}, f_i\rangle}{\|f_i\|^2}f_i\right)\\
&= \frac{\langle b, e_{k+1}\rangle}{\|f_{k+1}\|}\left(v_{k+1}-\sum\limits_{i=1}^{k}\frac{\langle v_{k+1}, f_i\rangle}{\|f_i\|^2}\|f_i\|e_i\right)\\
&= \frac{\langle b, e_{k+1}\rangle}{\|f_{k+1}\|}\left(v_{k+1}-\sum\limits_{i=1}^{k}\frac{\langle v_{k+1}, f_i\rangle}{\|f_i\|}e_i\right)
\end{align*}
$$

Let $c=\frac{\langle b, e_{k+1}\rangle}{\|f_{k+1}\|}\in\mathbb R$ and $\alpha=\sum\limits_{i=1}^{k}\frac{\langle v_{k+1}, f_i\rangle}{\|f_i\|}e_i\in\text{span}(e_1,...,e_k)=\text{span}(v_1,...,v_k)\subset\text{span}(v_1,...,v_{k+1})$, so $\langle b, e_{k+1}\rangle e_{k+1} = c(v_{k-1}-\alpha)$ which is in $\text{span}(v_1,...,v_{k+1})$ since it is a linear combination of elements in the span of those vectors, so $b = a + c(v_{k-1}-\alpha)\in\text{span}(v_1,...,v_{k+1})$, meaning that $\text{span}(e_1,...,e_{k+1})\subseteq\text{span}(v_1,...,v_{k+1})$.

Now, suppose $b\in\text{span}(v_1,...,v_{k+1})$. Then, there exist scalars $c_1,...,c_{k+1}$ such that $$b=\sum\limits_{i=1}^{k+1}c_iv_i = c_{k+1}v_{k+1} + \sum\limits_{i=1}^{k}c_iv_i$$ Let $a = \sum\limits_{i=1}^{k}c_iv_i\in\text{span}(v_1,...,v_k)=\text{span}(e_1,...,e_k)\subset\text{span}(e_1,...,e_{k+1})$ so $b= a + c_{k+1}v_{k+1}$. Observe that

$$
\begin{align*}
b&=a + c_{k+1}\left(f_{k+1} + \sum\limits_{i=1}^{k}\frac{\langle v_{k+1}, f_i\rangle}{\|f_i\|^2}f_i\right)\\
&= a+ c_{k+1}\left(\|f_{k+1}\|e_{k+1} +\sum\limits_{i=1}^{k}\frac{\langle v_{k+1}, f_i\rangle}{\|f_i\|^2}\|f_i\|e_i \right)\\
&= a+ c_{k+1}\left(\|f_{k+1}\|e_{k+1} +\sum\limits_{i=1}^{k}\langle v_{k+1}, e_i\rangle e_i \right)
\end{align*}
$$

Thus, $b\in\text{span}(e_1,...,e_{k+1})$, so $\text{span}(e_1,...,e_k)=\text{span}(v_1,...,v_k)$.

Since $\{e_1,...,e_n\}$ is an orthonormal set, it is therefore a linearly independent set and thus a basis of the span of its vectors which is equal to the span of the $v_i$'s, which is equal to $V$. Thus, $\{e_1,...,e_n\}$ is an orthonormal basis of $V$, which is what we wanted to show. $\blacksquare$
:::
